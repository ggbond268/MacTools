...
final class PluginRuntimeLocalizationTests: XCTestCase {
    ...
    func testCLINotarizationRecoveryGuidanceFollowsRuntimeLanguage() {
        let expectations = [
            ("en", "CLI notarization could not be confirmed. Check your network and retry, or update MacTools."),
            ("ar", "تعذّر تأكيد توثيق CLI لدى Apple. تحقق من اتصال الشبكة وحاول مجددًا، أو حدّث MacTools."),
            ("zh-Hans", "无法确认 CLI 的公证状态，请检查网络后重试，或更新 MacTools。"),
            ("zh-Hant", "無法確認 CLI 的公證狀態，請檢查網路後重試，或更新 MacTools。"),
        ]
    ...
    func testCLIInstallerCopyAndExistingFailureSwitchLanguagesAtRuntime() {
        ...
        XCTAssertEqual(CLIInstallCopy.confirmTitle.text, "安装 CLI？")
        setRuntimePreference("en")
        XCTAssertEqual(CLIInstallCopy.confirmTitle.text, "Install CLI?")
        ...
        setRuntimePreference("ar")
        XCTAssertEqual(CLIInstallCopy.confirmTitle.text, "هل تريد تثبيت CLI؟")
        XCTAssertEqual(CLIInstallCopy.status(.installed), "CLI مثبّت")
        setRuntimePreference("zh-Hant")
        XCTAssertEqual(CLIInstallCopy.confirmTitle.text, "安裝 CLI？")
