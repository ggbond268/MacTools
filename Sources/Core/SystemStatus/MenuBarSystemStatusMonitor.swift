...
@MainActor
protocol MenuBarNetworkPathMonitoring: AnyObject {
...
@MainActor
final class SystemMenuBarNetworkPathMonitor: MenuBarNetworkPathMonitoring {
    var onChange: (@Sendable (
        MenuBarSystemStatusSnapshot.Network,
        MenuBarSystemStatusSnapshot.ConnectionKind?
    ) -> Void)?
    ...
    func start(queue: DispatchQueue) {
        ...
        monitor.pathUpdateHandler = { path in
            let status: MenuBarSystemStatusSnapshot.Network = switch path.status {
            case .satisfied: .connected
            case .unsatisfied: .disconnected
            case .requiresConnection: .requiresConnection
            @unknown default: .unknown
            }
            let connectionKind: MenuBarSystemStatusSnapshot.ConnectionKind?
            if status != .connected {
                connectionKind = nil
...
@MainActor
final class MenuBarSystemStatusMonitor: ObservableObject {
    typealias LocalReader = @Sendable () -> MenuBarLocalSystemStatus
    typealias NotificationSourceFactory = (UnsafeMutableRawPointer) -> CFRunLoopSource?

    @Published private(set) var snapshot: MenuBarSystemStatusSnapshot = .unknown

    private let reader: LocalReader
    private let networkMonitorFactory: () -> any MenuBarNetworkPathMonitoring
    private let notificationSourceFactory: NotificationSourceFactory
    private let workspaceNotificationCenter: NotificationCenter
    private let refreshInterval: TimeInterval
    private let queue = DispatchQueue(label: "com.mactools.menu-bar-system-status", qos: .utility)
    private var networkMonitor: (any MenuBarNetworkPathMonitoring)?
    ...
    init(
        reader: @escaping LocalReader = MenuBarSystemStatusReader.read,
        networkMonitorFactory: @escaping () -> any MenuBarNetworkPathMonitoring = {
            SystemMenuBarNetworkPathMonitor()
        },
        notificationSourceFactory: @escaping NotificationSourceFactory = { context in
            IOPSNotificationCreateRunLoopSource({ context in
                ...
                let monitor = Unmanaged<MenuBarSystemStatusMonitor>
                    .fromOpaque(context)
                    .takeUnretainedValue()
    ...
    private func publish(_ updated: MenuBarSystemStatusSnapshot) {
