...
Published revisions and release artifacts that predate the GPL transition remain available under the license terms that accompanied those revisions, including Apache License 2.0. Copyright permissions already granted under Apache-2.0 for pre-transition code and contributions remain in effect and are not revoked by their inclusion in later GPL distributions. A copy of that license is retained at [`LICENSES/Apache-2.0.txt`](LICENSES/Apache-2.0.txt).
...
Official MacTools plugins are distributed under `GPL-3.0-only`. Official plugin release archives generated by this repository receive a copy of the root license during packaging; plugin source directories do not maintain duplicate license files.
