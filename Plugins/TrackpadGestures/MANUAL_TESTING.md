...
| Test Gestures | Switch between Test All and Practice One; start and stop while contacts are active; switch directly to another plugin page; test three-, four-, and five-finger double taps | The virtual trackpad follows live contacts, Practice One shows only the selected production guide and current mapping, no configured action executes, and testing stops automatically when the plugin page is no longer selected | Not run |
