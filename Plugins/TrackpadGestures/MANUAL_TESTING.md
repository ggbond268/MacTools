...
| Native one-finger double-click | Enable left/right one-fixed-finger TipTap with tap to click enabled; double-tap Finder items and select words in a text editor, at fast and slow intervals | Ordinary clicks remain responsive, double-clicks work consistently, and no TipTap mapping runs; repeat after a failed TipTap and with an external mouse or virtual pointer connected | Not run |
...
| External mouse or virtual pointer | Connect another mouse or virtual HID pointer, then try both that pointer and a click-producing trackpad gesture | An idle extra pointer does not disable a uniquely qualified trackpad episode; clicks from the extra pointer pass through unchanged, and overlapping or otherwise ambiguous input fails open | Not run |
...
| Native dragging and selection | Fast and slow text selection, window and file dragging, and three-finger dragging enabled and disabled | A pending TipTap immediately yields to dragging; Down, Drag, and Up remain ordered; conflicting three-finger mappings fail safely; native dragging remains usable | Not run |
...
For each double tap, touch and fully release twice in quick succession; expect its configured action exactly once on the second release. With no matching single-tap mapping, neither native tap should click through after success, while an expired first tap must replay unchanged. With matching single- and double-tap mappings, expect the documented single action followed by the double action. Repeat in Test Gestures and confirm the first release reports the ordinary single tap while the second release reports only the double tap. A gap longer than about 320 ms, excessive movement, an overlong contact, or a finger rebound before full release must not recognize a double tap.
...
For every middle-click mapping—including all TipTap, multi-finger tap, double-tap, and long-touch variants—confirm that one recognition produces exactly one middle click. Connect an external mouse or virtual HID pointer and repeat: a uniquely qualified trackpad episode must continue to work while unrelated pointer clicks pass through unchanged. Click both devices during the same correlation window and confirm genuinely ambiguous input fails open without suppressing or rewriting the unrelated click.
