...
struct WindowSwitcherAllSpacesWindowSnapshot {
...
private struct WindowSwitcherLegacyWindowRecordSnapshot {
...
@MainActor
final class WindowSwitcherAllSpacesCatalog {
    var onChange: (() -> Void)?
    ...
    private let activationWindowSnapshotProvider: ((pid_t, WindowSwitcherAppEntry) -> WindowSwitcherAllSpacesWindowSnapshot?)?
    ...
    init(
        ...
        windowRecordRefreshTimeout: TimeInterval = WindowSwitcherAllSpacesCatalog.defaultWindowRecordRefreshTimeout,
        applicationProvider: @escaping (pid_t) -> WindowSwitcherApplicationControlling? = {
            NSRunningApplication(processIdentifier: $0)
        },
        activationWindowSnapshotProvider: ((pid_t, WindowSwitcherAppEntry) -> WindowSwitcherAllSpacesWindowSnapshot?)? = nil,
    ...
    private func freshWindowRecordSnapshot() async -> WindowSwitcherLegacyWindowRecordSnapshot {
        refreshWindowRecords()
        guard windowRecordRefreshTask != nil else {
            return WindowSwitcherLegacyWindowRecordSnapshot(records: windowRecords, isFresh: false)
        }
        let generation = windowRecordRefreshGeneration
        ...
        return WindowSwitcherLegacyWindowRecordSnapshot(
    ...
    static func windowEntries(
        ...
        windows: [WindowSwitcherAllSpacesWindowSnapshot]
    ) -> [WindowSwitcherAppEntry] {
    ...
    private func windows(
        ...
        var axSnapshots: [WindowSwitcherAllSpacesWindowSnapshot] = []
    ...
    static func mergeWindowSnapshots(
        axSnapshots: [WindowSwitcherAllSpacesWindowSnapshot],
        ...
        var merged: [WindowSwitcherAllSpacesWindowSnapshot] = []
    ...
    private static func matches(
        _ snapshot: WindowSwitcherAllSpacesWindowSnapshot,
        title: String?,
        bounds: CGRect
    ) -> Bool {
    ...
    private static func windowSnapshot(
        ...
        return WindowSwitcherAllSpacesWindowSnapshot(
            element: window,
            windowNumber: nil,
    ...
    static func firstMatchingWindow(
        ...
        deadline: Date,
        now: () -> Date,
        snapshotProvider: (AXUIElement) -> WindowSwitcherAllSpacesWindowSnapshot?
    ) -> WindowSwitcherAllSpacesWindowSnapshot? {
        var matchingSnapshot: WindowSwitcherAllSpacesWindowSnapshot?
