...
final class DuoStatusIconTests: XCTestCase {
    func testDefaultArtworkIsTemplateAtStandardSize() throws {
        let image = DuoStatusIcon.image(for: connectedSnapshot)
    ...
    func testBatteryWiFiAndNetworkChangeIndependently() throws {
        let baseline = try alphaPixels(DuoStatusIcon.image(for: connectedSnapshot))
        var battery = connectedSnapshot
        battery.battery = .level(fraction: 0.1, isCharging: false)
        let batteryPixels = try alphaPixels(DuoStatusIcon.image(for: battery))
        ...
        let wifiPixels = try alphaPixels(DuoStatusIcon.image(for: wifi))
        ...
        let networkPixels = try alphaPixels(DuoStatusIcon.image(for: network))
    ...
    func testChargingAndMissingBatteryHaveDistinctArtwork() throws {
        ...
        XCTAssertNotEqual(try alphaPixels(DuoStatusIcon.image(for: charging)), try alphaPixels(DuoStatusIcon.image(for: connectedSnapshot)))
        XCTAssertNotEqual(try alphaPixels(DuoStatusIcon.image(for: missing)), try alphaPixels(DuoStatusIcon.image(for: empty)))
    }
    ...
    func testEthernetAndOtherRoutesDoNotRenderAsWiFi() throws {
        ...
        let wifi = try alphaPixels(DuoStatusIcon.image(for: connectedSnapshot))
        XCTAssertNotEqual(wifi, try alphaPixels(DuoStatusIcon.image(for: ethernet)))
        XCTAssertNotEqual(wifi, try alphaPixels(DuoStatusIcon.image(for: other)))
    }
    ...
    func testExternalPowerShowsPlugWhenChargingIsPausedOrComplete() throws {
        for fraction in [0.8, 1.0] {
            ...
            let unpluggedPixels = try alphaPixels(DuoStatusIcon.image(for: unplugged))
            let pluggedInPixels = try alphaPixels(DuoStatusIcon.image(for: pluggedIn))
    ...
    func testColoredRingPreservesBlackAndWhiteSymbolsInEachAppearance() throws {
        ...
        let light = DuoStatusIcon.image(for: snapshot, appearance: .light)
        let dark = DuoStatusIcon.image(for: snapshot, appearance: .dark)
    ...
    private var connectedSnapshot: DuoSystemStatusSnapshot {
