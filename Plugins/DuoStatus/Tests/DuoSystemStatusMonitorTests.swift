...
final class DuoSystemStatusMonitorTests: XCTestCase {
    ...
    func testRestartUsesFreshPathMonitorAndIgnoresOldCallbacks() async {
        let first = DuoNetworkPathMonitorFake()
        let second = DuoNetworkPathMonitorFake()
        var monitors = [first, second]
        let status = DuoLocalSystemStatus(battery: .notPresent, wifi: .off)
    ...
    func testStopDiscardsInFlightLocalReadingWithoutBlockingMainActor() async {
        let reader = BlockingDuoStatusReader()
        let monitor = DuoSystemStatusMonitor(
            reader: { reader.read() },
            networkMonitorFactory: { DuoNetworkPathMonitorFake() },
            notificationSourceFactory: { _ in nil },
            workspaceNotificationCenter: NotificationCenter()
        )
    ...
    func testDeinitRemovesPowerSourceAndWakeObserver() async throws {
        ...
        var monitor: DuoSystemStatusMonitor? = DuoSystemStatusMonitor(
            reader: { DuoLocalSystemStatus(battery: .notPresent, wifi: .off) },
...
private final class BlockingDuoStatusReader: @unchecked Sendable {
    ...
    func read() -> DuoLocalSystemStatus {
