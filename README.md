...
| Clipboard History | Keep searchable text, rich text, images, PDFs, audio, video, links, colors, and grouped file references in an encrypted local store while preserving their original pasteboard representations. A first-run guide explains storage, opening history, and the two clearly labeled sensitive-copy workflows: Private Copy Now copies in one step, while Ignore Next Copy covers a later context-menu copy without reading it. Pause collection, exclude applications by best-effort frontmost-app context, and confirm privacy or oversized-content events through a non-activating HUD. Choose a count limit or no item-count limit, an expiration period or no expiration, and a per-item embedded-content limit while a fixed encrypted-store budget keeps resource use bounded. The draggable, borderless, search-first history panel follows the Command Palette design language, supports spaced or compact prefix matching such as `fo baz` or `foba` for `foo bar baz`, indexes text in images on-device with visible recognition status, and offers one-click type filters with Control-1 through Control-8 shortcuts. Minimal two-line rows keep source context on the left, quick-paste position at the top-right, and relative time at the bottom-right; content length, dimensions, page or file counts, file size, duration, format, and link host appear only in a compact lazily loaded detail line. Exact detail timestamps precede concise, single-unit relative times. Finder-copied PDFs, images, audio, and video remain native file references for reliable paste while also appearing in both Files and their semantic type filter; single supported files receive a thumbnail preview. Rich-text previews load off the main UI path on a switchable light or dark paper canvas, while embedded images use a bounded transparency-aware canvas. Navigate with arrows or Control-P/Control-N, press Return to paste, Shift-Return to paste a plain-text conversion such as rich text or recognized image text, Command-1 through Command-9 to paste a numbered result, or Command-Delete to remove the selection without jumping to the top. Reused items move to the top the next time the panel opens. Press the Open Clipboard History global shortcut again to dismiss its active panel. Plugin settings use three shortcut groups: primary access for Open Clipboard History and Paste Current Clipboard as Plain Text, sensitive-copy workflows, and optional advanced collection or clearing controls. The plain-text shortcut can also paste completed on-device text recognition from the still-current clipboard image without opening history, while a HUD distinguishes recognition in progress from an image with no recognized text. Native file-URL pasteboard objects keep grouped file pasting compatible with Finder. Copy, pin, or delete items without adding clipboard payloads to Unified Search or action details. |
...
| Auto Input | Remember and restore input sources per app, assign fixed sources, switch directly with global shortcuts, and optionally show the current source whenever an editable field gains focus—including fixed-source apps, accessory palettes such as Raycast, and MacTools' own Settings and global command searches. The transient HUD anchors near the caret without covering the full input control and can stay visible while hovered for repeated-click source cycling. An optional quieter mode keeps first-focus and input-source-change prompts immediate, then reminds again after either a configurable interval or number of app switches. |
