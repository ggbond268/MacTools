...
| 启动台           | 全屏或紧凑窗口唤出应用网格，支持即时搜索、横向分页、键盘导航与全局快捷键，并对中文/日文输入法组字安全。                    |
...
> **macOS 27 说明：** 新的单窗口菜单栏宿主不再通过常规通道把右键事件传递给第三方菜单栏图标，且当前测试版还会把图标的窗口尺寸上报为退化值（零高度），因此只读事件监听只能尽力尝试、无法稳定救活右键。请用 **Option+左键** 点击 MacTools 图标打开次级面板——这是 macOS 27 上可靠的方式。
