...
| 触控板手势       | 仅添加需要的 TipTap、三至五指轻点、双击和长触手势，并映射到键盘快捷键或鼠标中键；TipTap 可在固定手指保持按住时反复触发，并提供无障碍控件、内置 TipTap 指引、唯一手势映射、快捷键复用提示和不执行操作的测试模式，原中键设置会自动迁移。 |
| 隐藏刘海         | 自动遮挡内建刘海屏顶部区域，不修改用户原始壁纸。                                                                          |
| 隐藏菜单栏图标   | 通过菜单栏分割符隐藏左侧图标，支持拖动调整显示/隐藏/永久隐藏；点击分割符可在聚合浮窗中临时显示隐藏图标。 |
...
| 窗口布局         | 用半屏、三分、四分、最大化等预设调整前台窗口大小与位置，支持全局快捷键与恢复原位置。                                      |
...
| 系统状态         | 展示 CPU、GPU、内存、磁盘、网络、电量与高占用进程的近 1 小时图表；默认在菜单栏显示 CPU/内存/网络，CPU 卡片附带开机时长。 |
...
| 电池充电上限     | 限制电池充电至指定上限（默认 80%），达到上限后停止充电；支持下限提醒、热保护、休眠停充策略，并显示健康度/循环次数/温度。电量低于上限时不自动恢复，由用户决定何时继续充电或强制放电。 |
...
| 插件与设置       | 在任一 MacTools 界面按 ⌘K、点击插件侧边栏的搜索入口，或为「打开命令面板」配置可选全局快捷键，即可打开统一搜索面板，查找应用页面、通用设置、插件、具体插件设置项和明确支持的命令；全局快捷键默认不分配，配置后可从任意应用打开独立的键盘优先面板。命令可明确选择跟随系统、深色或浅色外观，开启或关闭登录时启动，并按插件支持情况在仪表盘或功能面板中显示或隐藏插件；只提供会改变当前状态的操作。可用 ↑/↓ 与 Return，或按 ⌘1–⌘9 快速打开前九项可见结果，辅助功能标签和深层链接会直接定位并聚焦目标设置。⌘F 继续用于插件市场等当前页面的局部搜索。插件设置由宿主统一渲染 macOS 分组表单、语义背景、卡片、权限和快捷键区域，复杂管理器也复用同一页面外壳；较长选项会使用紧凑下拉菜单，步进滑块不显示密集刻度并实时更新读数。通过菜单栏面板顶部居中的切换器选择功能或组件面板。在插件市场中安装、更新和批量更新插件（支持分类/搜索筛选；可按未安装优先、已安装优先或按所选语言的名称排序），并在各插件设置页维护权限、快捷键（支持单独使用 F1-F12 功能键；常见应用快捷键会先提示其可能全局抢占）和专属设置；还可设置全局快捷键打开设置窗口、命令面板、仪表盘或功能面板。 |
...
> **快捷键设置：** 插件与应用快捷键会将操作图标、名称和录入框紧凑排列在同一行；分组在空间不足时按完整快捷键行换行，不会拆散单个控件。
