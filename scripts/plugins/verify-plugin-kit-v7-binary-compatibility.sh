...
FIXTURE_DIR="$REPO_ROOT/scripts/fixtures/plugin-kit-v7"
...
TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/mactools-plugin-kit-v7.XXXXXX")"
trap 'rm -rf "$TEMP_DIR"' EXIT

TARGET="$(uname -m)-apple-macos14.0"
MODULE_PATH="$TEMP_DIR/MacToolsPluginKit.swiftmodule"
CLIENT_PATH="$TEMP_DIR/plugin-kit-v7-client"
...
print -r -- "PluginKit v7 binary compatibility client passed."
