...
The preview viewport owns an AppKit magnification recognizer, including its image and surrounding margins. Native event-dispatch tests cover pinch increments through the chooser panel; physical trackpad acceptance remains separate. Preview inspection stays within the existing viewport: pinch to zoom up to 4×, drag or use arrow keys while the enlarged preview has focus to pan, and double-click to fit. The preview context menu and options menu offer Zoom In, Zoom Out, and Fit Preview. Search and Select also supports Command-Plus, Command-Minus, and Command-0 unless reserved by existing Direct Keys assignments. Selecting another window resets the preview to Fit and immediately disables interaction with the outgoing image. Its pixels blur immediately, hold for 50 ms, and fade out over 40 ms while the next capture is scheduled; a ready replacement cancels that fade immediately. Transitions never queue and Reduce Motion removes both effects.
...
The visible preview acquires application focus for native trackpad gestures in either invocation mode. Cancelling restores the original application only while the chooser still owns focus; selecting a window or switching to another application does not restore it.
