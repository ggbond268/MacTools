...
The public Astro site is generated from the same validated `Plugins/*/plugin.json` source manifests and does not fetch a production catalog while building. Run `cd site && npm run generate:plugins` after changing a manifest or Marketplace asset; commit `site/src/generated/*.json` and any checksum-named files under `site/public/generated/plugin-assets/`. `npm run check:generated-plugins` fails when those artifacts are stale. Plugin pages use `mactools://app/settings/plugins/marketplace/<plugin-id>` links, and static-action pages add a paired `provider` and `action` highlight; neither link installs or executes an action.
