...
`Plugins/MenuBarHidden/` restores the `menu-bar-hidden` plugin for PluginKit v7 under `GPL-3.0-only`. It hides menu-bar items to the left of a divider and provides visible, hidden, and always-hidden layout areas. The component panel can show captured hidden icons and forward clicks to their original menu-bar items.
