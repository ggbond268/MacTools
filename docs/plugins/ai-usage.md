...
AI Usage (`ai-usage`) is a PluginKit v6 plugin for MacTools 1.3.1 or later. It provides a Dashboard component, a feature-panel Dashboard button, and a declarative settings form. The first version supports one current Codex login and one current Claude Code login.
