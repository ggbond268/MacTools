...
- **Scrolling screenshots** sample the selected region while you scroll it yourself. Finish to copy and pin the assembled image, or cancel to discard it. Matching and composition run on a serial background worker. Newly appended strips own their pixels instead of retaining full capture frames. Output is limited to 128 MiB of RGBA pixels and 32,768 rows; reaching a limit stops capture with guidance to use smaller sections. Capture failures are reported; results arriving after the session ends are ignored.
