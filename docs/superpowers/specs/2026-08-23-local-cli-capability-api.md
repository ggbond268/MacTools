...
declares `.cancellable`. Disconnecting a CLI cancels requests that have not yet
been durably accepted. A continuing action that has already returned `started`
keeps its host-owned progress UI and is not canceled by client disconnect.
...
6. The primary host registers with the broker only after plugin loading,
   `ActionRegistry` synchronization, and workflow action publication complete.
...
- If `~/.local/bin` is not on `PATH`, Settings shows the appropriate shell line
  ...
  shell configuration.
- Advanced users may invoke the bundled path directly.
...
- interrupt propagation only for cancellable work
