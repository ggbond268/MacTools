...
- 在菜单栏插件列表中新增一行「显示器分辨率」，点击标题行展开/收起内嵌列表
- 列出所有活跃显示器，每个显示器一个 section，section 内是按分辨率由高到低排序的单列表
- 同宽高比筛选：默认仅显示与显示器原生比例一致的模式，但始终保留当前生效项
...
- 不支持镜像拓扑管理
- 不承诺支持虚拟/Dummy 显示器（DisplayLink、Sidecar 等）；若公共 API 将其暴露为可用桌面显示器，则按 best-effort 方式展示
...
| 展开状态 | `isExpanded` + `.setDisclosureExpanded(Bool)` | 保持 `isOn` 仅表示插件 active，避免污染宿主状态 |
| 列表内容 | 单一列表 + 同宽高比筛选 + 强制保留当前项 | UI 简洁，同时始终能看到当前 checkmark |
...
Sources/
  Core/
    Plugins/
      PluginModels.swift           # 【改】新增 .disclosure / .selectList / isExpanded / sectionTitle / descriptionTone
  Features/
  ...
  App/
    MenuBarContent.swift           # 【改】新增 header-only disclosure 交互 + .selectList 渲染 + error tone
  Core/
    Diagnostics/
...
| UI | `MenuBarContent`（增量） | 渲染 `.disclosure` 行、`.selectList` 控件与错误态文案颜色 | SwiftUI |
...
- **`isOn` 只表示插件 active**：展开态单独走 `isExpanded`，不能复用现有 active 语义
...
enum DisplayResolutionError: Error, LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .displayUnavailable: return "显示器已断开连接"
...
var panelState: PluginPanelState {
    ...
    return PluginPanelState(
        ...
        isOn: false,                       // 分辨率插件不是 active/inactive 语义
...
private func buildDetail(for displays: [DisplayInfo]) -> PluginPanelDetail {
    let controls = displays.flatMap { display -> [PluginPanelControl] in
        ...
        let visibleModes = Self.visibleModes(modes)

        let options = visibleModes.map { mode in
            PluginPanelControlOption(
                id: String(mode.modeId),
                title: Self.optionTitle(for: mode)
            )
        }
        let currentID = visibleModes.first(where: { $0.isCurrent }).map { String($0.modeId) }
...
static func visibleModes(
    ...
    return modes.filter { mode in
        abs(mode.aspectRatio - nativeAspect) < 0.01 || mode.isCurrent
    }
}
...
func handlePanelAction(_ action: PluginPanelAction) {
    switch action {
    case let .setDisclosureExpanded(isExpanded):
    ...
    case let .setSelection(controlID, optionID):
        ...
        guard controller.listConnectedDisplays().contains(where: { $0.id == displayID }) else {
            handleApplyFailure(
                .displayUnavailable(displayID: displayID),
                displayID: displayID,
                modeId: modeId
            )
            return
    ...
    case .setSwitch:
        return  // 不使用，保留给 switch 型 plugin
    case .setDate:
        return  // 不使用
    }
}
...
enum PluginPanelAction {
    case setSwitch(Bool)
    case setDisclosureExpanded(Bool)   // 【新】仅 .disclosure 使用
...
enum PluginControlStyle {
    case `switch`
    case disclosure   // 【新】行右侧显示 chevron，标题行可点击
}
...
struct PluginPanelState {
    let subtitle: String
    let isOn: Bool
    let isExpanded: Bool          // 【新】仅 .disclosure 使用，默认 false
    let isEnabled: Bool
    let isVisible: Bool
...
struct PluginPanelItem: Identifiable {
    ...
    let helpText: String
    let descriptionTone: PluginPanelDescriptionTone   // 【新】错误态渲染红字
    let isOn: Bool
    let isExpanded: Bool
    let isEnabled: Bool
    let detail: PluginPanelDetail?
...
`KeepAwakePlugin` / `PhysicalCleanModePlugin` 的 `panelState` 都需要显式回填 `isExpanded: false`，保持 `isOn` 仍然只表示 active 状态。
...
`PluginHost` 需要新增 `setDisclosureExpanded(_:for:)`，内部转发到 `plugin.handlePanelAction(.setDisclosureExpanded(...))`；`hasActivePlugin` 与 feature management active dot 仍只看 `panelState.isOn`。

`PluginHost.rebuildDerivedState()` 需要补两处映射：
...
**为什么新增 `.setDisclosureExpanded(Bool)` / `isExpanded` 而不复用 `isOn`**：
- 当前宿主已把 `isOn` 用于菜单栏 icon 与设置页 active dot，不能引入第二套语义
- 展开态是纯 UI 状态，和插件是否“正在运行”没有关系
- blast radius 最小：只给 `.disclosure` 行新增分支，不影响现有 switch plugin

**为什么用 `descriptionTone` 而不是单独做一个 error widget**：
- 继续复用现有 feature row 描述区，不新增额外 layout
- 只扩展颜色语义，最小化 `PluginHost` 与 `MenuBarContent` 改动面
- 未来如果需要 warning/success 文案，也可在同一通道扩展
...
`.disclosure` 行只允许标题区域响应展开/收起。做法是把当前 header `HStack` 提取成 `rowHeader`，仅在 `.disclosure` 时用 `Button` 包裹该 header；detail 区保持在 `Button` 外，避免和列表项点击手势冲突。
...
`FeatureRowView` 新增 `onDisclosureToggle: (Bool) -> Void` 回调；`MenuBarContent` 透传给 `PluginHost.setDisclosureExpanded(_:for:)`。`.switch` 样式保留现状（由 Toggle 自己处理手势）。
...
- feature row 错误文案沿用原位置，仅颜色切为 `Color.red`
...
| 菜单已展开后目标显示器被拔掉 | plugin 侧先命中 `.displayUnavailable`，统一走 `handleApplyFailure`，UI 在行 description 红字显示 |
| `applyResolution` 前 `modeId` 失效 | `.failure(.modeNotFound)` 或 plugin 侧预检失败后统一走 `handleApplyFailure`，UI 在行 description 红字显示 |
...
| 虚拟/Dummy/DisplayLink | MVP 不做 vendorID/transport 黑名单；若公共 API 仍枚举出 safe + desktop GUI 模式，则按 best-effort 处理，不作为兼容性承诺 |
...
- 选择负载非法（controlID / optionID 解析失败）：error 级
- 切换开始：info 级，带目标 `width×height@modeId`
- 切换失败：error 级，带 `displayID`、`modeId` 与失败原因（`CGError.rawValue` 或逻辑错误类型）
- 切换成功：info 级
...
| `test_visibleModes_dropsOffRatioWhenNotCurrent` | `DisplayResolutionPlugin.visibleModes` | 混合 16:9 和 16:10，且异形项非 current → 只保留 native 同比项 |
| `test_visibleModes_keepsCurrentEvenWhenOffRatio` | 同上 | 当前项为 16:10，其余为 16:9 → 返回结果仍包含 current 项 |
| `test_visibleModes_emptyInput` | 同上 | `[]` → `[]` |
| `test_DisplayResolutionInfo_equatable_byModeId` | `DisplayResolutionInfo.==` | 同 modeId 不同宽高 → 相等 |

`optionTitle` / `visibleModes` / `parseDisplayID` 设为 `internal static` 方法，测试目标通过 `@testable import MacTools` 访问。
...
1. **单屏内建 MacBook Retina**：
   - [ ] 插件行显示主屏当前分辨率
   - [ ] 点击展开，列表项数量 ≥ 3
   - [ ] 展开/收起插件行不会点亮菜单栏 active 图标，也不会让设置页该插件出现 active dot
   - [ ] 当前项有 checkmark
   - [ ] 选中其他项，屏幕切换 ≈ 1s，成功后 checkmark 跳转
   - [ ] 点击展开后的列表项不会意外收起整个 section
   - [ ] Log 中看到成功记录
...
6. 当前生效模式即使不符合 native 宽高比筛选，也必须保留在 `visibleModes` 中，否则 UI 会丢当前 checkmark
7. 镜像拓扑不由本插件管理，如果未来要同步改镜像组需额外循环
8. `.permanently` 写系统偏好，用户下次插相同显示器会记得——这是预期行为
