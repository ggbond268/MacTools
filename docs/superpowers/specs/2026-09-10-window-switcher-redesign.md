...
Tracks #414 (roadmap), #415 (reliability), and #416 (navigation). This branch implements tracking and a searchable grid/list chooser with an optional selected-window preview. Multiple simultaneous previews and native Command-Tab augmentation remain follow-ups. The current implementation is described below; dated validation sections record historical checkpoints, not acceptance of the latest head. Host shortcut-conflict handling and the plugin must ship together.
...
- Deliberately typing or focusing Search keeps the chooser open after modifier release. Scope and layout changes preserve the current cycling or search mode; there is no timed promotion. Clearing search does not re-enable release-to-switch. Typing from a held-modifier cycle session removes the invocation modifiers until their release, then restores ordinary editing modifiers. Native text editing owns paste, selection, and IME composition. Escape during composition belongs to the input method; otherwise it cancels.
- The nonactivating panel preserves the foreground app in every presentation mode. An explicit preview click may acquire application focus for native trackpad gestures without changing the invocation mode; showing a preview alone does not activate the host. Cancelling restores the original app only if the chooser acquired application focus and still owns it; committing a selection or moving to another app does not restore it. The bounded list reveals the selection on navigation or filter changes and preserves manual scrolling during background metadata refreshes. Stable window identity preserves selection through metadata changes; ordering remains frozen within a session, with new arrivals appended.
...
- Close Window and Quit App are separate explicit commands. Requests are not reported as completed disappearance. Save/cancel dialogs remain owned by the target app. Existing profiles retain Direct Keys and their editable assignments. Search and Select is the new-install default; changing modes is explicit. Existing assignments never silently become Close Window or Quit App commands.
...
## Current review and release boundaries

The implementation includes the plugin redesign, host shortcut-conflict handling, settings picker alignment, diagnostics, and compatibility-build-path support. Shared global-panel APIs are additive in MacTools 1.3.1 / PluginKit v6; see [global panel presentation](../../plugins/global-panel-presentation.md). The optional private AX/window-ID and Space-membership adapters remain isolated and fail conservatively when unavailable.

The local integration build has been installed and exercised for adaptive grid/list sizing, scope clicks, Light/Dark scope appearance, and preview zoom/Fit. These checks do not substitute for testing the standalone PR merge. Hosted CI must pass on the final head before the draft is marked ready.

Physical IME candidate selection, VoiceOver navigation, macOS 14/26 appearance, fullscreen and Spaces/Stage Manager transitions, multiple displays/disconnection, and save/cancel dialogs remain explicitly scoped manual acceptance work before release. They are not represented as completed by the synthetic tests or the historical Chrome diagnostics below. Pinch zoom is bounded and keyboard/menu alternatives remain available.
...
- Label All Windows and Current App shortcut recorders separately. Chooser-local Command-Shift-1/2 changes scope, Command-Option-1/2 changes grid/list, Command-1 through Command-9 opens numbered visible results in Search and Select, Command-D opens the display menu, Command-P toggles preview, and Command-F focuses search. Scope and presentation controls preserve cycling; focusing Search makes the session persistent. These controls respect native marked-text handling.
