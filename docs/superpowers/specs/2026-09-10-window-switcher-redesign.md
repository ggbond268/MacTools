...
- The nonactivating panel preserves the foreground app when no preview is shown. A visible preview explicitly acquires application focus for native trackpad gestures without changing the invocation mode. Cancelling restores the original app only while the chooser still owns focus; committing a selection or moving to another app does not restore it. The bounded list reveals the selection on navigation or filter changes and preserves manual scrolling during background metadata refreshes. Stable window identity preserves selection through metadata changes; ordering remains frozen within a session, with new arrivals appended.
...
The PR includes the plugin redesign, host shortcut-conflict handling, settings picker alignment, diagnostics, and compatibility-build-path support. No new public PluginKit API is introduced; the plugin requires MacTools 1.3.1 / PluginKit v6. The optional private AX/window-ID and Space-membership adapters remain isolated and fail conservatively when unavailable.
