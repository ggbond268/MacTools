...
- `PluginActionInputPresentationRequesting` is an optional MacTools 1.3.0 host-routing capability. Requests must belong to the live requesting provider and resolve to a registered input descriptor. Panel routing uses the existing app presentation owner and standalone palette lifecycle.
