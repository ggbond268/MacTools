...
- Register each new public symbol in the minimum-host compatibility inventory. Set Siri's `minHostVersion` to the first host release actually shipping this API; the implementation requires `1.3.1` / PluginKit v6 because released MacTools `1.3.0` does not export the input APIs.
...
## Full-review fixes and Siri panel composer (2026-09-09)

- The native search field uses a dedicated composition-aware field editor. Composition updates disable submission before preedit text is published and restore submission only after committed text is current. Inline submission also checks the native editor directly. A native mouse probe reproduced the original incomplete send and now blocks it until composition commits.
- Inline actions revalidate the current alias mapping before sending and again after asynchronous preparation. Registry changes preserve the displayed target while marking removed or conflicting mappings unavailable. Bare-alias Return and Edit follow the same validation; user text is retained.
- Siri window-control searches skip `chatSessionView` transcript descendants while retaining the composer inside `innerChatSessionView`. Delivery evidence skips assistant-response descendants while preserving strict reads of the chat container and user messages. Large/deep response tests retain duplicate-control, missing-data, traversal-bound, and deadline protections.
- The Siri feature-panel row now offers Ask Siri while idle and Cancel while sending. Ask Siri requests the existing host composer for the registered action. Opening input does not open Siri, create a conversation, or send a message. Failed and uncertain attempts retain their explanation; opening the composer is never an automatic retry.
- `PluginActionInputPresentationRequesting` is an optional MacTools 1.3.1 host-routing capability. Requests must belong to the live requesting provider and resolve to a registered input descriptor. Panel routing uses the existing app presentation owner and standalone palette lifecycle.
