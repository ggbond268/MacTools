...
First-party artwork recovered from repository history is not a third-party source and stays out of `sources/manifest.json`; record its origin here instead. `static-mactools-legacy-t` reproduces the original MacTools menu-bar mark from commit `ce39bc66`, rasterized from that commit's app-icon artwork as black template artwork on a transparent 96×96 canvas.
