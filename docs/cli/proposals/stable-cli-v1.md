...
Status: direction approved; implementation and release validation are in progress. Stable publication remains disabled. See the [signed Nightly acceptance report](../validation/2026-09-10-nightly-signed-acceptance.md) for completed checks and remaining gaps.
...
The repository owner approved the separate optional download and General-settings installation flow in [the proposal discussion](https://github.com/ggbond268/MacTools/issues/417#issuecomment-5628700782), including Apple silicon initially and the app's minimum macOS version (currently macOS 14). [The follow-up on #418](https://github.com/ggbond268/MacTools/pull/418#issuecomment-5630219005) requested a replacement PR after an accidental merge and revert, and reiterated that validation must finish before stable publication. The combined implementation PR restores this proposal and its sanitized acceptance report alongside stable support.
...
- Begin with Apple silicon, as approved. Intel support is a separate decision requiring its own artifact and acceptance coverage.
- Follow the app's minimum macOS version, currently macOS 14, as approved. Test it in addition to macOS 26 and 27 before claiming signed stable acceptance.
...
- [x] Agree on initial platforms: Apple silicon and the app's minimum macOS version.
- [ ] Validate the entire advertised platform matrix, including macOS 14.
...
- [x] Obtain maintainer approval for the separate optional stable distribution policy.
- [ ] Obtain release-candidate approval after completing validation.
...
2. Record the approved direction and platform commitments; complete the compatibility review.
...
Stable candidates with sealed CLI metadata use `~/.local/bin/mactools` and a publisher-scoped directory under `Library/Application Support/MacTools/CLI`. Nightly retains `mactools-nightly` and its existing store. App identity, manifest channel, receipt ownership, and immutable release URL must agree. Both channels retain the existing confirmation, integration permission, managed updates, rollback, and removal behavior.

The stable release workflow defaults `STABLE_CLI_ENABLED` to `false`. Ordinary stable builds omit CLI metadata and retain the existing manual integration settings. A maintainer can request a candidate-only build for validation; that run cannot publish releases or update feeds. Changing the default requires a reviewed follow-up after the remaining acceptance checks pass. See the [release procedure](../../plugins/cli-release.md).
